...
- precoContratual: 7 rows (0.003%) are negative, from -207,000.0 to -2,997.0. Checked all 7 individually — procedure/contract types are mixed (2 Concessão de serviços públicos, 2 Aquisição de serviços, 1 Empreitadas de obras públicas, others), so a single "concession payment direction" explanation doesn't cover the group. At least 2 of the 7 (e.g. idcontrato 14810012: precoContratual -18063.5 against precoBaseProcedimento +18063.5, same magnitude, opposite sign) look like a sign-entry error rather than a legitimate negative value, but this isn't confirmed.
- precoBaseProcedimento: 1 row negative (-5400.0, idcontrato 11220782), a Concessão de serviços públicos, where a negative base price is at least plausible given how concession fee direction can run — not confirmed either, just less inexplicable than the precoContratual cases.
